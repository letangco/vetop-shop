...
export const isStore = new CreateAuth('store', jwt_user.getJWTKey(), 'bearer', async (token, done) => {
  try {
    ...
    if (token.access === 'staff') {
      const staff = await getStaffInfo(token?.staffId || '');
      if (!staff) {
        return done(null, false);
      }
      return done(null, {
...
export const isAuthorized = () => async (req, res, next) => {
  ...
  if (authorizationArray[0] === 'bearer') {
    ...
    if (userData.access === 'staff') {
      const staff = await getStaffInfo(userData?.staffId || '');
      if (!staff._id) {
        return next(new APIError(401, 'Unauthorized'));
      }
      req.user = {
...
export const isOwnerStore = new CreateAuth('ownerStore', jwt_user.getJWTKey(), 'bearer', async (token, done) => {
  try {
    // eslint-disable-next-line new-cap
    if (token.access !== 'store') {
      return done(null, false);
